...
This project has a dependency on a gem that is internal to NSIDC.  In order to
successfully build the project, you will need to remove the following lines from
the Gemfile:
...
Acceptance tests are a little trickier.  One way to run them on your local dev
machine is thus:

1. Change `config/app_config.rb` and set the `development`:`solr_url` to point
   to the same as the `integration`:`solr_url` (it's a url on liquid for NSIDC
   internal use)
...
Guard can be used to automatically restart the puma server or run RuboCop and
unit tests whenever a file changes.
...
Truslove, Jonathan Kovarik, Luis Lopez, Michael Brandt, Stuart Reed (2014):
NSIDC Open Search. The National Snow and Ice Data
Center. Software. http://ezid.cdlib.org/id/doi:10.7265/N51R6NG0
