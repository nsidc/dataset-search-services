    ...
    module Search
      class SolrSearchStandard < SolrSearchBase
        ...
        def build_q_parameter(search_params)
          ...
          if generic_terms.empty? && field_terms.empty?
            ...
            operator_joined_terms(
              build_generic_terms(generic_terms) <<
                build_spatial_term(field_terms) <<
        ...
        def build_temporal_term(field_terms)
          ...
          # The start date needs to be converted to a latitude and will have the
          #   format YY.YYMMDD
          # The end date needs to be converted to a longitude and will have the
          #   format YY.YYMMDD
          if field_terms.key?(:startDate) && field_terms.key?(:endDate)
            ...
            # 90,end_date, which means search every thing with an end date after
            # the start date.
            'temporal:['\
              "#{format_temporal_start(field_terms[:startDate])},0 TO "\
              "90,#{format_temporal_end(field_terms[:endDate])}"\
              ']'
          elsif field_terms.key? :startDate
            # Solr indexes temporal ranges as points in a spatial field. To find
            # all datasets with ranges that intersect the start date create a
            # query with the format [start_date,0 TO 90,180], which means search
            # every thing with an end date after the start date.
            "temporal:[#{format_temporal_start(field_terms[:startDate])},0 TO 90,180]"
          elsif field_terms.key? :endDate
            # Solr indexes temporal ranges as points in a spatial field. To find
            # all datasets with ranges that intersect the end date, create a
            # query with the format [0,0 TO 90,enddate], which means search
            # every thing with a start date before the end dates.
